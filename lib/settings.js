...
exports.markets_page = {
  ...
  //            Supported exchanges: dex-trade, dexomy, freiexchange/freixlite, nestex, nonkyc, poloniex, qutrade, yobit
  "exchanges": {
    ...
    //         NOTE: nestex does not display a 24-hour chart due to a lack of OHLCV api data
    "nestex": {
...
exports.api_cmds = {
  ...
  "getblock": "getblock",
  // getrawtransaction: Returns raw transaction data. Can return a hex-encoded string that is serialized or an object with txid information depending on the decrypt value (0 = false or 2 = true)
...
exports.blockchain_specific = {
  ...
  // coinstake: A collection of settings that pertain to PoS (proof-of-stake) coinstake transactions.
  //            These options only affect how the explorer displays PoS blocks/transactions and do
  //            not change what data is stored. Already-synced blocks/txs will pick up the changes
  //            immediately on the next page render (no reindex required).
  "coinstake": {
    // dev_fund_address: The address that receives the optional dev-fund output on PoS coinstake
    //                   blocks (see blackcoin-more src/wallet/staking.cpp CreateCoinStake). When
    //                   set, this address is excluded from the "Extracted By" list on PoS blocks
    //                   and transactions so the dev fund is not mis-attributed to a staker.
    //                   Leave blank for chains that do not have a dev fund.
    "dev_fund_address": ""
  }
};
