...
function get_extracted_by_addresses(show_extracted_by, internal, tx, cb) {
  ...
  if (show_extracted_by == true) {
    // check if this is a coinbase/coinstake tx
    if (
      tx != null &&
      tx.vout != null &&
      (
        ...
        // PoS coinstake: vin holds the kernel entry (kept for balance tracking
        // in block_sync) so the empty-vin/coinbase check above won't match.
        // detect PoS via tx_type instead.
        (tx.tx_type != null && tx.tx_type.indexOf('pos') !== -1)
      )
    ) {
      // for PoS coinstakes (tx_type includes 'pos'), filter out the minter key
      // (0-value vout) and the configured dev_fund_address so only the staker's
      // stake-refund address(es) are returned. For PoW coinbases, all vout
      // addresses are returned as before.
      ...
      if (tx.tx_type != null && tx.tx_type.indexOf('pos') !== -1) {
        ...
        extracted_by_addresses = tx.vout
