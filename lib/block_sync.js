...
function fix_address_data(address_data, cb) {
  const amount = new Decimal(address_data.amount.toString());
  let addr_inc = {};

  // determine how to fix the address balances
  if (address_data.a_id == 'coinbase')
    addr_inc.sent = mongoose.Types.Decimal128.fromString(amount.negated().toString());
  else if (amount.lt(0)) {
    // vin
    addr_inc.sent = mongoose.Types.Decimal128.fromString(amount.toString());
    addr_inc.balance = mongoose.Types.Decimal128.fromString(amount.negated().toString());
  } else {
    // vout
    addr_inc.received = mongoose.Types.Decimal128.fromString(amount.negated().toString());
    addr_inc.balance = mongoose.Types.Decimal128.fromString(amount.negated().toString());
  }
...
function hex_to_ascii(hex) {
  ...
  for (let i = 0; i < hex.length; i += 2) {
    ...
    str += char;
    if (byte < 0x20 && byte !== 0x09 && byte !== 0x0A && byte !== 0x0D) allPrintable = false;
    if (byte > 0x7E) allPrintable = false;
  }

  return allPrintable ? str : hex;
}
...
function update_addresses(addresses, blockheight, txid, cb) {
  const bulkAddresses = addresses.map((address) => ({
    updateOne: {
      ...
      update: {
        ...
        $inc: {
          sent: mongoose.Types.Decimal128.fromString(address.sent.toString()),
          balance: mongoose.Types.Decimal128.fromString(address.balance.toString()),
          received: mongoose.Types.Decimal128.fromString(address.received.toString())
        }
      },
      upsert: true
    }
  ...
  const bulkAddressTxes = addresses
    ...
    .map((address) => {
      // amount is Decimal.js; apply sign before converting
      const signedAmount = (address.type === 'vin')
        ? address.amount.negated()
        : address.amount;
      ...
      return {
        updateOne: {
          ...
          update: {
            ...
            $inc: { amount: mongoose.Types.Decimal128.fromString(signedAmount.toString()) }
          },
          upsert: true
        }
      };
  ...
  save_tx: function(txid, blockheight, block, cb) {
    try {
      lib.get_rawtransaction(txid, function(tx) {
        if (
          tx &&
          tx != `${settings.localization.ex_error}: ${settings.localization.check_console}` &&
                ...
                nvin.forEach(function(input) {
                  ...
                  if (input.addresses) {
                    ...
                    if (index == -1) {
                      ...
                      addressBatch[index].amount = addressBatch[index].amount.add(input.amount);
                      addressBatch[index].sent = addressBatch[index].sent.add(sent);
                      addressBatch[index].balance = addressBatch[index].balance.add(balance);
                    }
                  }
                });
                ...
                vout.forEach(function(output) {
                  ...
                  if (output.addresses) {
                    ...
                    if (index == -1) {
                      ...
                      // address already exists
                      addressBatch[index].amount = addressBatch[index].amount.sub(output.amount);
                      addressBatch[index].balance = addressBatch[index].balance.add(balance);
                      addressBatch[index].received = addressBatch[index].received.add(received);
                    }
                  }
                });
                    ...
                    function flushTxBatch(txBatchCallback) {
                      ...
                      if (!localTxBatch || localTxBatch.length === 0) {
                        ...
                        const bulkTxes = localTxBatch.map(tx => {
                          ...
                          // ensure the total field is cast as a mongo Decimal128 value before saving
                          plainTx.total = mongoose.Types.Decimal128.fromString(plainTx.total.toString());
    ...
    AddressTx.find({txid: txid}).exec().then((address_txes) => {
      if (address_txes.length == 0) {
        ...
        Tx.findOne({txid: txid}).then((tx) => {
          ...
          // check if this is a coinbase tx (coinstakes have empty vin but should not be treated as coinbase)
          if (tx.vin == null || tx.vin.length == 0) {
            if (tx.tx_type == null || tx.tx_type.indexOf('pos') === -1) {
              ...
              addressTxArray.push({
                txid: txid,
                a_id: 'coinbase',
                amount: new Decimal(tx.total.toString())
              });
            }
          }
          ...
          if (tx.vin != null && tx.vin.length > 0) {
            ...
            for (var vin_tx_counter = tx.vin.length - 1; vin_tx_counter >= 0; vin_tx_counter--) {
              // loop through the addresstx data
              for (var vin_addresstx_counter = address_txes.length - 1; vin_addresstx_counter >= 0; vin_addresstx_counter--) {
                ...
                if (tx.vin[vin_tx_counter].addresses == address_txes[vin_addresstx_counter].a_id && new Decimal(tx.vin[vin_tx_counter].amount.toString()).eq(new Decimal(address_txes[vin_addresstx_counter].amount.toString()).negated())) {
                  // add the address into the addressTxArray array
                  addressTxArray.push({
                    txid: txid,
                    a_id: tx.vin[vin_tx_counter].addresses,
                    amount: new Decimal(address_txes[vin_addresstx_counter].amount.toString())
                  });
          ...
          if (tx.vout != null && tx.vout.length > 0) {
            ...
            for (var vout_tx_counter = tx.vout.length - 1; vout_tx_counter >= 0; vout_tx_counter--) {
              ...
              for (var vout_addresstx_counter = address_txes.length - 1; vout_addresstx_counter >= 0; vout_addresstx_counter--) {
                // check if there is a tx record that exactly matches to the addresstx
                if (tx.vout[vout_tx_counter].addresses == address_txes[vout_addresstx_counter].a_id && new Decimal(tx.vout[vout_tx_counter].amount.toString()).eq(new Decimal(address_txes[vout_addresstx_counter].amount.toString()))) {
                  // add the address into the addressTxArray array
                  addressTxArray.push({
                    txid: txid,
                    a_id: tx.vout[vout_tx_counter].addresses,
                    amount: new Decimal(address_txes[vout_addresstx_counter].amount.toString())
                  });
          ...
          if (tx.vin.length > 0 || tx.vout.length > 0 || address_txes.length > 0) {
            ...
            address_list.forEach(function(address) {
              let vin_total = new Decimal('0');
              let vout_total = new Decimal('0');
              let address_tx_total = new Decimal('0');

              // add up all the vin amounts for this address
              tx.vin.forEach(function(vin) {
                // check if this is the correct address
                if (vin.addresses == address)
                  vin_total = vin_total.add(new Decimal(vin.amount.toString()));
              });
              ...
              tx.vout.forEach(function(vout) {
                ...
                if (vout.addresses == address)
                  vout_total = vout_total.add(new Decimal(vout.amount.toString()));
              });
              ...
              address_txes.forEach(function(address_tx) {
                ...
                if (address_tx.a_id == address)
                  address_tx_total = address_tx_total.add(new Decimal(address_tx.amount.toString()));
              });

              // check if the tx and addresstx totals match
              if ((vout_total.sub(vin_total)).eq(address_tx_total)) {
                ...
                address_txes.forEach(function(address_tx) {
                  ...
                  if (address_tx.a_id == address) {
                    ...
                    addressTxArray.push({
                      txid: txid,
                      a_id: address,
                      amount: new Decimal(address_tx.amount.toString())
                    });
                  }
                });
              }
