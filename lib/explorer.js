...
function coinbase_supply(cb) {
  Address.findOne({a_id: 'coinbase'}).then((address) => {
    if (address)
      return cb(new Decimal(address.sent.toString()));
    else
      return cb(new Decimal('0'));
  }).catch((err) => {
    console.log(err);
    return cb(new Decimal('0'));
  });
...
function processVoutAddresses(address_list, vout_value, arr_vout) {
  ...
  if (address_list != null && address_list.length > 0) {
    ...
    const amount_sat = module.exports.convert_to_satoshi(new Decimal(vout_value.toString()));
    ...
    if (index == -1) {
      ...
      // already exists
      arr_vout[index].amount = arr_vout[index].amount.add(amount_sat);
    }
  }
...
function get_input_addresses(input, vout, cb) {
  ...
  if (input.coinbase) {
    ...
    for (let i = 0; i < vout.length; i++)
      amount = amount.add(new Decimal(vout[i].value.toString()));
        ...
        async.eachSeries(tx.vout, function(current_vout, loop) {
          if (current_vout.n == input.vout) {
            if (current_vout.scriptPubKey.addresses || current_vout.scriptPubKey.address) {
              ...
              if (index == -1)
                addresses.push({hash: new_address, amount: new Decimal(current_vout.value.toString())});
              else
                addresses[index].amount = addresses[index].amount.add(new Decimal(current_vout.value.toString()));
              ...
              if (settings.blockchain_specific.bitcoin.enabled == true) {
                ...
                encodeP2PKaddress(current_vout.scriptPubKey.asm, function(p2pkh_address) {
                  ...
                  if (p2pkh_address != null) {
                    ...
                    if (index == -1)
                      addresses.push({hash: p2pkh_address, amount: new Decimal(current_vout.value.toString())});
                    else
                      addresses[index].amount = addresses[index].amount.add(new Decimal(current_vout.value.toString()));
                    ...
                    if (index == -1)
                      addresses.push({hash: 'unknown_address', amount: new Decimal(current_vout.value.toString())});
                    else
                      addresses[index].amount = addresses[index].amount.add(new Decimal(current_vout.value.toString()));
...
function finalize_vout(first_vout, arr_vout, arr_vin, tx_type, txid, cb) {
  if (typeof first_vout !== 'undefined' && first_vout.scriptPubKey.type == 'nonstandard') {
    if (arr_vin.length > 0 && arr_vout.length > 0) {
      // find the vout that matches the kernel address (the stake output).
      // it may not be the first non-OP_RETURN vout if there are extra marker outputs.
      //
      // blackcoin-more coinstake vout layout (see src/wallet/staking.cpp CreateCoinStake):
      //   vout[0]: OP_RETURN (empty script) -- always, marks this as a coinstake
      //   vout[1]: P2PK "minter key" output (value 0) -- ONLY when kernel is bech32/bech32m
      //            (witness_v0_keyhash / witness_v1_taproot), since those script types
      //            can't sign coinstakes yet. the wallet signs using a separate P2PK key.
      //   vout[1 or 2]: stake refund back to the kernel address (kernel + reward)
      //   vout[2 or 3]: optional second stake output (split when nCredit >= 1000 BLK)
      //   vout[last]:  optional dev-fund output
      //
      // legacy (P2PKH/P2PK) coinstakes have no minter key, so the stake output is vout[1].
      // bech32/bech32m coinstakes have a minter key at vout[1] and the stake output at vout[2..3].
      let stake_vout_index = -1;

      for (let i = 0; i < arr_vout.length; i++) {
        if (arr_vout[i].addresses == arr_vin[0].addresses) {
          stake_vout_index = i;
          break;
        }
      }

      if (stake_vout_index >= 0) {
        // mark this as a pos coinstake (distinct from coinbase/poW) so the tx
        // view can render the proof_of_stake label and the API/block sync can
        // distinguish it from a regular coinbase tx
        tx_type = (tx_type == null ? 'pos' : tx_type + ',pos');

        // The stake refund vout is (kernel + reward). For the staker's
        // accounting we want the kernel to be a wash (it was already received
        // in some previous tx, then spent here, then received back as part of
        // this vout). only the reward portion should count as new "received".
        //
        // To achieve that:
        //   - subtract the kernel from the stake refund vout, so arr_vout only
        //     credits the reward to the kernel address (received += reward)
        //   - shift the kernel out of arr_vin so block_sync does not debit it
        //     from sent (sent += 0)
        //
        // Net effect per coinstake: balance change = +reward, which matches
        // the actual UTXO balance change for the staker. the kernel itself
        // is a wash, as it should be.
        arr_vout[stake_vout_index].amount = arr_vout[stake_vout_index].amount.sub(arr_vin[0].amount);
        arr_vin.shift();
...
module.exports = {
  convert_to_satoshi: function(amount) {
    // multiply by 1e8 and round to nearest (half up)
    return amount.times('1e8').toDecimalPlaces(0, Decimal.ROUND_HALF_UP);
  },
  ...
  get_hashrate: function(cb) {
    ...
    if (settings.shared_pages.page_header.panels.network_panel.nethash == 'netmhashps') {
      ...
      const cmd = prepareRpcCommand(settings.api_cmds.getmininginfo);
        ...
        if (settings.api_cmds.use_rpc) {
          ...
          rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (response.netmhashps) {
              ...
              net_hash = new Decimal(response.netmhashps.toString()).mul(1000000);
            } else if (response.networkhashps)
              net_hash = new Decimal(response.networkhashps.toString());
            else if (response.hashespersec)
              net_hash = new Decimal(response.hashespersec.toString());
          ...
          request({uri: base_url + 'getmininginfo', json: true}, function (error, response, body) {
            ...
            if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`) {
              ...
              if (body.netmhashps) {
                // value returned in MH/s so convert to H/s
                net_hash = new Decimal(body.netmhashps.toString()).mul(1000000);
              } else if (body.networkhashps)
                net_hash = new Decimal(body.networkhashps.toString());
              else if (body.hashespersec)
                net_hash = new Decimal(body.hashespersec.toString());
      ...
      const cmd = prepareRpcCommand(settings.api_cmds.getnetworkhashps);
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        ...
        if (settings.api_cmds.use_rpc) {
          ...
          rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (response) {
              // return hash value with proper units
              return cb(convertHashUnits(new Decimal(response.toString())));
            } else {
          ...
          request({uri: base_url + 'getnetworkhashps', json: true}, function (error, response, body) {
            ...
            if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`) {
              ...
              // return hash value with proper units
              return cb(convertHashUnits(new Decimal(body.toString())));
            }
          });
        }
      } else {
  ...
  get_difficulty: function(cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getdifficulty);
  ...
  get_connectioncount: function(cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getconnectioncount);
  ...
  get_masternodelist: function(cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getmasternodelist);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getmasternodelist', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_masternodecount: function(cb) {
    ...
    if (
      settings.api_page.public_apis.rpc.getmasternodecount.enabled == true &&
      settings.api_cmds['getmasternodecount'] != null &&
      settings.api_cmds['getmasternodecount'] != ''
    ) {
      const cmd = prepareRpcCommand(settings.api_cmds.getmasternodecount);
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        if (settings.api_cmds.use_rpc) {
          ...
          request({uri: base_url + 'getmasternodecount', json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              return cb(null);
            else
  ...
  get_blockcount: function(cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getblockcount);
  ...
  get_blockhash: function(height, cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getblockhash, (height ? [parseInt(height)] : []));
  ...
  get_block: function(hash, cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getblock, (hash ? [hash] : []));
  ...
  get_rawtransaction: function(hash, cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getrawtransaction, (hash ? [hash, 1] : []));
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getrawtransaction?txid=' + hash + '&decrypt=2', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_maxmoney: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getmaxmoney);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getmaxmoney', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_maxvote: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getmaxvote);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getmaxvote', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_vote: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getvote);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getvote', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_phase: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getphase);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getphase', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_reward: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getreward);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getreward', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_estnext: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getnextrewardestimate);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getnextrewardestimate', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_nextin: function(cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getnextrewardwhenstr);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getnextrewardwhenstr', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_descriptorinfo: function(descriptor, cb) {
    ...
    const cmd = prepareRpcCommand(settings.blockchain_specific.bitcoin.api_cmds.getdescriptorinfo, (descriptor ? [descriptor] : []));
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getdescriptorinfo?descriptor=' + encodeURIComponent(descriptor), json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  get_deriveaddresses: function(descriptor, cb) {
    const cmd = prepareRpcCommand(settings.blockchain_specific.bitcoin.api_cmds.deriveaddresses, (descriptor ? [descriptor] : []));
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'deriveaddresses?descriptor=' + encodeURIComponent(descriptor), json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  balance_supply: function(cb) {
    Address.find({}, 'balance').where('balance').gt(0).exec().then((docs) => {
      ...
      async.eachSeries(docs, function(current_doc, loop) {
        count = count.add(current_doc.balance.toString());
        loop();
      }, function() {
        return cb(count);
      });
  ...
  get_supply: function(cb) {
    if (settings.sync.supply == 'HEAVY') {
      // attempt to get the supply from the getsupply or similar api cmd that returns the current
      // money supply as a single positive decimal value
      const cmd = prepareRpcCommand(settings.blockchain_specific.heavycoin.api_cmds.getsupply);
        ...
        if (settings.api_cmds.use_rpc) {
          rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (response == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              ...
              return cb(response ? new Decimal(response.toString()) : null);
          });
        } else {
          request({uri: base_url + 'getsupply', json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              return cb(null);
            else
              return cb(body ? new Decimal(body.toString()) : null);
          });
        }
      } else {
        // cmd not in use. return null.
      ...
      // attempt to get the supply from the getinfo or similar api cmd that returns and object
      // containing various state info. Must include a value called "moneysupply" which represents the current running total of coins
      const cmd = prepareRpcCommand(settings.api_cmds.getinfo);
      ...
      if (!(cmd.method == '' && cmd.parameters.length == 0)) {
        if (settings.api_cmds.use_rpc) {
          ...
          request({uri: base_url + 'getinfo', json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (!body || !body.moneysupply || body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              return cb(null);
            else
              return cb(new Decimal(body.moneysupply.toString()));
          });
        }
      } else {
        // cmd not in use. return null.
      ...
      // get the supply by running a query on the addresses collection and summing up all
      // positive balances (potentially a long running query for blockchains with tons of addresses)
      module.exports.balance_supply(function(supply) {
        return cb(supply.div(100000000));
      });
    } else if (settings.sync.supply == 'TXOUTSET') {
      // attempt to get the supply from the gettxoutsetinfo or similar api cmd that returns
      // an object with statistics about the unspent transaction output set. Must include
      // a value called "total_amount" which represents the current running total of coins
      const cmd = prepareRpcCommand(settings.api_cmds.gettxoutsetinfo);
        ...
        if (settings.api_cmds.use_rpc) {
          rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (!response || !response.total_amount || response == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              ...
              return cb(new Decimal(response.total_amount.toString()));
          });
        } else {
          request({uri: base_url + 'gettxoutsetinfo', json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (!body || !body.total_amount || body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              return cb(null);
            else
              return cb(new Decimal(body.total_amount.toString()));
          });
        }
      } else {
        // cmd not in use. return null.
      ...
      // attempt to get the supply from the getblockchaininfo or similar api cmd that
      // returns and object containing various state info. Must include a value called
      // "moneysupply" which represents the current running total of coins
      const cmd = prepareRpcCommand(settings.api_cmds.getblockchaininfo);
        ...
        if (settings.api_cmds.use_rpc) {
          rpcCommand([{method:cmd.method, parameters: cmd.parameters}], function(response) {
            ...
            if (!response || !response.moneysupply || response == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              ...
              return cb(new Decimal(response.moneysupply.toString()));
          });
        } else {
          request({uri: base_url + 'getblockchaininfo', json: true}, function (error, response, body) {
            // check if an error msg was received from the web api server
            if (!body || !body.moneysupply || body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
              return cb(null);
            else
  ...
  get_peerinfo: function(cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.getpeerinfo);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        request({uri: base_url + 'getpeerinfo', json: true}, function (error, response, body) {
          // check if an error msg was received from the web api server
          if (body == `${settings.localization.ex_error}: ${settings.localization.check_console}`)
            return cb(null);
          else
  ...
  verify_message: function(address, signature, message, cb) {
    const cmd = prepareRpcCommand(settings.api_cmds.verifymessage, [address, signature, message]);
    ...
    if (!(cmd.method == '' && cmd.parameters.length == 0)) {
      if (settings.api_cmds.use_rpc) {
        ...
        const uri = base_url + 'verifymessage?address=' + address + '&signature=' + signature + '&message=' + message;
  ...
  calculate_total: function(vout) {
    ...
    for (let i = 0; i < vout.length; i++)
      total = total.add(new Decimal(vout[i].amount.toString()));
          ...
          if (current_vout.scriptPubKey.type != 'zerocoinmint') {
            ...
            if (address_list == null || address_list.length == 0) {
              ...
              if (current_vout.scriptPubKey.address == null) {
                ...
                // check if bitcoin features are enabled
                if (settings.blockchain_specific.bitcoin.enabled == true) {
                  // assume the asm value is a P2PK (Pay To Pubkey) public key that should be encoded
                  // as a P2PKH (Pay To Pubkey Hash) address
                  encodeP2PKaddress(current_vout.scriptPubKey.asm, function(p2pkh_address) {
                    // check if the address was encoded properly
                    if (p2pkh_address != null) {
                      // mark this tx as p2pk
        ...
        if (settings.blockchain_specific.zksnarks.enabled == true && vhidden != null && vhidden.length > 0) {
          ...
          async.eachSeries(vhidden, function(current_vhidden, loop) {
            if (current_vhidden.vpub_old > 0) {
              // process vout addresses and save the updated array
              arr_vout = processVoutAddresses([hidden_address], current_vhidden.vpub_old, arr_vout);

              // move to next vout
              loop();
            } else {
              const amount_sat = module.exports.convert_to_satoshi(current_vhidden.vpub_new);
              ...
              if ((!vout || vout.length == 0) && (!vin || vin.length == 0)) {
                ...
                if (index == -1) {
                  ...
                  arr_vin[index].amount = arr_vin[index].amount.add(amount_sat);
                }
    ...
    async.eachSeries(tx.vin, function(vin, loop) {
      get_input_addresses(vin, tx.vout, function(addresses, tx_type_vin) {
        ...
        if (addresses && addresses.length) {
          const amount_sat = module.exports.convert_to_satoshi(addresses[0].amount);
          ...
          if (index == -1)
            arr_vin.push({addresses: 'unknown_address', amount: new Decimal('0')});
  ...
  is_locked: function(lock_array, silent = false) {
    ...
    for (var i = 0; i < lock_array.length; i++) {
      const pidFile = path.join(path.dirname(__dirname), 'tmp', `${lock_array[i]}.pid`);

      // check if the script is already running (tmp/file.pid file already exists)
      if (fs.existsSync(pidFile)) {
        const { execSync } = require('child_process');
        let deactivateLock = false;
        ...
        switch (process.platform) {
          case 'win32':
            ...
            const cmdResult = execSync(`tasklist /FI "PID eq ${fs.readFileSync(pidFile).toString()}"`);

            // check if the process that created the lock is actually still running
            // (crude check by testing for # of carriage returns or node.exe process running,
            // but should work universally across different systems and languages)
          ...
          default:
            ...
            try {
              execSync('ps -p `cat "' + pidFile + '"` > /dev/null');
            } catch (err) {
  ...
  format_decimal_string: function(dec, { minFractionDigits = 2, maxFractionDigits = 8 } = {}) {
    // dec is a Decimal instance
    let s = dec.toFixed(maxFractionDigits); // string with up to maxFractionDigits
    ...
    return neg ? `-${int}${frac ? '.'+frac : ''}` : `${int}${frac ? '.'+frac : ''}`;
  }
};